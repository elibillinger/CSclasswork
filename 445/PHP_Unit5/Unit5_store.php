...
<form id="form" action="Unit5_process_order.php" method="post">
    ...
    <input type="submit" value="Purchase" onclick="setCookie()">
</form>
<div id="product">
...
<script>
    function showImage(){
        ...
        var part = $("#part option:selected").attr('value');
        if(!viewed.includes(part)){
            viewed.push(part);
        }
        $("#temp").hide();
        $("img")
    ...
    function setCookie() {
        var part = $("#part option:selected").attr('value');
        var index = viewed.indexOf(part);
        if(index !== -1){
            viewed.splice(index, 1);
        }
        var json_str = JSON.stringify(viewed);
        var cookie = "viewed-items=" + json_str;
