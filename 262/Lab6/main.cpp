...
void stl_sort(vector<unsigned int> &vec);
...
int main() {
    ...
    while (true) {
        ...
        // Algorithm choice
        int algorithm = 0;
        while (algorithm < 1 || algorithm > 4 || !cin) {
            if (!cin) {
                cin.clear();
                string discard;
                getline(cin, discard);
            ...
            cout << "4 - C++ standard library sort" << endl;
            cin >> algorithm;
        }
        cout << endl;
        ...
        if (algorithm == 4) cout << "C++ standard library sort";
        cout << " on ";
...
/*
 * void stl_sort(vector<unsigned int> &vec)
 * Sorts a vector of integers in place using C++ standard library.
 */
void stl_sort(vector<unsigned int> &vec) {
