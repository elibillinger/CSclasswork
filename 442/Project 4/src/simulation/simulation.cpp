...
Simulation::Simulation(FlagOptions flags) {
    ...
    // TODO: Add your other algorithms as you make them
       ...
       this->scheduler = std::make_shared<CFSScheduler>();
        if(flags.time_slice > 3){
            this->scheduler->min_granularity = flags.time_slice;
        } else {
             this->scheduler->min_granularity = 3;
        }  
    } else {
...
SystemStats Simulation::calculate_statistics() {
    for(auto i: processes){
        for(auto j:  i.second->threads) {
            ...
            int tt = j->turnaround_time();
            this->system_stats.avg_thread_turnaround_times[j->priority] += tt;
            int rt = j->response_time();
            this->system_stats.avg_thread_response_times[j->priority] += rt;
        } 
    }
    for(int i = 0; i < 4; ++i){
...
std::shared_ptr<Thread> Simulation::read_thread(std::istream& input, int thread_id, int process_id, ProcessPriority priority) {
    ...
    this->system_stats.thread_counts[priority]++;
